...
- (void)createUserCouponListInfoUI
{
    ...
    ///边框
    UIView *lineRootView = [[UIView alloc] initWithFrame:CGRectMake(SIZE_DEFAULT_MARGIN_LEFT_RIGHT, 5.0f, SIZE_DEFAULT_MAX_WIDTH, 60.0f)];
    ...
    ///标题
    self.titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.couponImage.frame.origin.x + self.couponImage.frame.size.width + 5.0f, self.couponImage.frame.origin.y + 5.0f, lineRootView.frame.size.width - self.couponImage.frame.origin.x - self.couponImage.frame.size.width - 5.0f, 25.0f)];
    self.titleLabel.font = [UIFont boldSystemFontOfSize:16.0f];
    self.titleLabel.textColor = COLOR_HEXCOLOR(0x939598);
    [lineRootView addSubview:self.titleLabel];
    
    ///有效期
    self.dateLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.titleLabel.frame.origin.x,self.titleLabel.frame.origin.y + self.titleLabel.frame.size.height + 5.0f, self.titleLabel.frame.size.width, 25.0f)];
